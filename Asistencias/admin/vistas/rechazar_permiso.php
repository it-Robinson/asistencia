...
$sql = "UPDATE solicitudes_permisos SET estado='Rechazado', usuario_revisor='$completoAprobador' WHERE id='$id'";
if (mysqli_query($conn, $sql)) {
    echo "Permiso rechazado correctamente.";

    // Obtener información de la solicitud y del empleado
    $sql_info = "SELECT u.email, u.nombre, u.apellidos, s.fecha_inicio, s.fecha_fin, s.motivo, s.descripcion 
    ...
    if ($result) {
        if (mysqli_num_rows($result) > 0) {
            ...
            try {
                ...
                $mail->isHTML(true);
                $mail->Subject = 'Solicitud de permiso rechazada';
                $mail->Body    = "Hola {$nombre} {$apellidos},<br><br>
                                  Tu solicitud de permiso ha sido <b>Rechazada</b> por <b>{$completoAprobador}</b>.<br><br>
                                  <b>**Por favor comunicarte con RRHH para saber el motivo.**</b><br><br>
                ...
                $mail->AltBody = "Hola {$nombre} {$apellidos},\n\n
                                  ...
                                  **Por favor comunicarte con RRHH para saber el motivo.**\n\n
